...
def detect_block(inputs,out_channel,name):
    x = tf.keras.layers.DepthwiseConv2D(3,1,padding='same',name ="dw_conv" + name,use_bias=False)(inputs)
    x = tf.keras.layers.BatchNormalization(name ="dw_conv_bn" + name)(x)
    x = tf.keras.layers.ReLU(6.0)(x)

    x = tf.keras.layers.Conv2D(out_channel,1,1,padding='same',name ="conv" + name,use_bias=False)(x)
    x = tf.keras.layers.BatchNormalization(name = "conv_bn" + name)(x)
    return x
    pass
...
def M2SSDNet(inputs_shape,net_params):
    ...
    for i, res in enumerate(stage_results):
        ...
        loc_pred = detect_block(stage_results[i],(len(anchor_sizes[i]) + len(net_params.anchor_ratios[i])) * 4,"loc_"+str(i))
        ...
        # class prediction
        cls_pred = detect_block(stage_results[i],(len(anchor_sizes[i]) + len(net_params.anchor_ratios[i])) * net_params.n_classes,"cls_"+str(i))
