...
from SSDUtil import non_max_suppression,draw_boxes,preprocess_img
...
def load_pre_model_for_voc():
    weights_path = os.path.join(log_dir, "ssdlite_coco_loss-4.8205_val_loss-4.1873.h5")
    model = M2SSDNet((batch_size, 300, 300, 3), params)
    model1 = M2SSD300Net_()
    model1.load_weights(weights_path, by_name=True)
    #print("=================================================")
    #i, j = 0, 0
    var, var1 = [], []
    for v, v1 in zip(model.variables, model1.variables):
        if (not "cls" in v.name):
            var.append(v)
            #i += 1
            pass
        if (not "cls" in v1.name):
            var1.append(v1)
            #j += 1
            pass
        pass
    ...
    for v, v1 in zip(var, var1):
        print(v.name + "__" + str(v.shape) + "===================" + v1.name + "__" + str(v1.shape))
        v.assign(v1.numpy())
        pass
    #save_path = os.path.join(log_dir, "ssdlite_voc.h5")
    #model.save_weights(save_path)
    # print("*****************************************************************")
    # for v, v1 in zip(model.variables, model1.variables):
    #     if "conv_bnloc_5/moving_variance" in v.name:
    #         print(v.numpy())
    #         print("*****************************************************************")
    #         pass
    #     if "ssd_box6conv2_bn/moving_variance" in v1.name:
    #         print(v1.numpy())
    #         print("*****************************************************************")
    #         pass
    #     pass
...
if __name__ ==  '__main__':
    weights_path = os.path.join(log_dir, "ssd_voc_87_loss-1.2861_val_loss-4.3294.h5")
    model = M2SSDNet((1, 300, 300, 3), params)
    # model1 = M2SSD300Net_()
    #model = load_pre_model_for_voc()
    #model1 = M2SSD300Net_((1, 300, 300, 3), params)
